...
- **🔥 進行中主線:itops × iTop(Combodo)真整合** — 見 `integration/itop/README.md`。
  iTop 3.2.2 已驗證能本機 Docker 站起+ITIL 安裝(101 表);下一步=啟 REST + 寫 `itop_sync.py`。
  iTop 容器已關;重站 ~5 分鐘(README 有指令)。
