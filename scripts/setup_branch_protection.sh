...
#   - required_status_checks:七道護欄必須通過才可合併(變更管理 / A.8.32)。
...
#       + promote-gate(Phase F 過版閘門,TASK-F3:擋跳關/夾私貨/未驗章)。
...
read -r -d '' BODY <<JSON || true
{
  "required_status_checks": {
    "strict": true,
    "contexts": ["policy-secrets", "policy-structure", "policy-iac", "cmdb-validate", "deploy-gate-selftest", "change-class", "promote-gate"]
  },
  "enforce_admins": true,
