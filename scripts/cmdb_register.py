...
def main() -> int:
    ...
    # ── 多層拓樸:真 CMDB 是「CI + 彼此關係」,記到 host → middleware → software 三層 ──
    #   software(app) --runs-on--> middleware(OpenLiberty) --hosted-on--> host(VM/容器宿主)
    # 部署目標型別:last-deploy.json 有 vmIp = 真 VM;否則視為容器宿主(podman)。
    is_vm = bool(record.get("vmIp"))
    host_addr = record.get("vmIp") or "127.0.0.1"
    host_url = record.get("url") or f"http://{host_addr}:{http_port}"
    ci_host = f"ci-host-{env}"
    ci_mw = f"ci-mw-{app}-{env}"
    ci_sw = f"ci-{app}-{env}"

    # — host CI(硬體層:VM / 容器宿主)—
    host_ci = {
        "apiVersion": "cmdb/v1", "kind": "ConfigurationItem",
        "metadata": {"ciId": ci_host, "type": "host", "app": app, "environment": env},
        "spec": {
            "attributes": {
                "kind": "qemu/libvirt VM" if is_vm else "container host (podman)",
                "address": host_addr,
                "provisionedBy": "OpenTofu",
                "iac": f"iac/environments/{env}",
                "target": record.get("target", "podman" if not is_vm else "qemu/libvirt"),
            },
            "relationships": [
                {"type": "provisioned-by", "target": f"iac/environments/{env}"},
                {"type": "hosts", "target": ci_mw},
            ],
        },
    }
    # — middleware CI(中介軟體層:OpenLiberty 執行期)—
    mw_ci = {
        "apiVersion": "cmdb/v1", "kind": "ConfigurationItem",
        "metadata": {"ciId": ci_mw, "type": "middleware", "app": app, "environment": env},
        "spec": {
            "attributes": {
                "product": "OpenLiberty",
                "runtime": runtime.get("type", "openliberty"),
                "instance": app,
                "httpPort": http_port,
                "url": host_url,
            },
            "relationships": [
                {"type": "hosted-on", "target": ci_host},
                {"type": "hosts", "target": ci_sw},
            ],
        },
    }
    # — software CI(軟體層:已部署的 app 實例)—
    sw_ci = {
        "apiVersion": "cmdb/v1", "kind": "ConfigurationItem",
        "metadata": {"ciId": ci_sw, "type": "deployed-application", "app": app, "environment": env},
        "spec": {
            "source": {
                "artifact": source.get("artifact"), "version": source.get("version"),
                "digest": source.get("digest"),
                "gitCommit": source.get("gitCommit", ""), "gitTag": source.get("gitTag", ""),
                "testReport": source.get("testReport", ""), "testCount": source.get("testCount", 0),
                "signature": sig_rel,
            },
            "runtime": {
                "type": runtime.get("type", "openliberty"), "instance": app,
                "url": host_url, "httpPort": http_port,
            },
            "provenance": {
                "serviceRequest": meta.get("serviceRequest", ""),
                "requestedBy": meta.get("requestedBy", ""),
                "deploymentRequest": deploy_req_rel,
                "gate": record.get("gate", ""), "deployedAt": record.get("deployedAt", ""),
            ...
            "relationships": [
                ...
                {"type": "runs-on", "target": ci_mw},   # ← 指向中介軟體 CI(非環境名)
            ],
        },
    }
    ...
    header = (
        ...
        "# 多層拓樸:host(VM/宿主)→ middleware(OpenLiberty)→ software(app),以 ciId 互連。\n"
        "# 版控史即組態基線與變更史。對應:ISO 20000 組態管理 / ISO 27001 A.8.9。請勿手改。\n"
    )
    for ci, name in ((host_ci, "host"), (mw_ci, f"{app}-middleware"), (sw_ci, app)):
        p = out_dir / f"{name}.yaml"
        p.write_text(header + yaml.safe_dump(ci, allow_unicode=True, sort_keys=False), encoding="utf-8")
    print(f"✅ 已登錄 3 層 CI:{out_dir}/  "
          f"(host {ci_host} → middleware {ci_mw} → software {ci_sw}, {source.get('digest')})")
    return 0
